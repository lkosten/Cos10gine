...
void BoardRayIterator::SetBoardRay(bitboard bb, RayDirection direction) {
    ray_direction = direction;
    current_ray = kRayPatterns[MS1BInd(bb)][direction];
}

bitboard BoardRayIterator::GetNextRaySquareBitboard() {
    bitboard next_ray_square_bb;
...
std::vector<bitboard> BoardRayIterator::Bitscan(bitboard bb) {
    std::vector<bitboard> res;
    res.reserve(2);

    while(bb) {
        res.push_back(LS1B(bb));
        bb ^= res.back();
    }
